...
- **저장 시점**: 별도의 "저장" 버튼 없이, 이름/버전/설명/실행 파일 경로 등 필드가 바뀔 때마다 즉시(짧은 debounce 후) `games.json`에 자동 저장한다 — video-vault의 `library.json`/`settings.json` 자동 저장과 동일한 패턴. 창을 닫을 때 "저장하지 않은 변경사항" 확인 절차는 없다(이미 저장되어 있으므로).
