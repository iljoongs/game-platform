...
- `ExecutablePath`가 비어 있거나 파일이 더 이상 존재하지 않으면 오류 메시지를 보여주고 실행하지 않는다.
...
- **게임 요약** — 캡처 이미지 갤러리 (`Screenshots`). 창 안에 이미지 파일을 드래그드롭하면 목록에 추가되고, 각 항목은 320x240 썸네일로 표시된다. 개별 삭제는 항목 우클릭으로 처리한다 (메인 화면 카드 삭제와 동일한 상호작용 규칙).

## 확인/결정이 더 필요한 사항

- 카드 삭제 시 연결된 이미지 파일(대표 썸네일/스크린샷 원본·리사이즈본)도 함께 지울지, 확인 대화상자를 띄울지.
- 게임 요약 갤러리 항목을 클릭했을 때 원본 크기로 크게 보여주는 뷰어(video-vault의 `OriginalImageWindow`처럼)를 둘지 여부.
- `ExecutablePath`가 가리키는 파일이 없어졌을 때 메인 화면 카드에 경고 표시를 할지 (video-vault의 `IsExist`와 유사한 개념).
- 정보 창을 게임별로 여러 개 동시에 열 수 있게 할지, 아니면 종류당 하나만 열리게 할지 ([공통 관리](common-management.md)의 창 관리 정책 참고).
